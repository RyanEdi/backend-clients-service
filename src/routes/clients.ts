...
const decryptClientRow = (row: Record<string, any>): Record<string, any> => ({
  ...
  dataNascimento: row.data_nascimento ? decryptField(String(row.data_nascimento)) : row.data_nascimento,
  email: row.email ? decryptField(String(row.email)) : row.email,
  phone: row.phone ? decryptField(String(row.phone)) : row.phone,
  zipCode: row.cep ? decryptField(String(row.cep)) : row.cep,
  address: row.endereco_completo ? decryptField(String(row.endereco_completo)) : row.endereco_completo,
  rg: row.rg ? decryptField(String(row.rg)) : row.rg,
  cidadeUf: row.cidade_uf ? decryptField(String(row.cidade_uf)) : row.cidade_uf,
  tipoDeficiencia: row.tipo_deficiencia ? decryptField(String(row.tipo_deficiencia)) : row.tipo_deficiencia,
